...
(* Main entry point - only run if invoked directly *)
let main () =
  if Array.length Sys.argv < 2 then begin
    Printf.printf "Usage: %s file1.vhd [file2.vhd ...]\n" Sys.argv.(0);
    Printf.printf "\nExamples:\n";
    Printf.printf "  %s sysver_tests/slib_input_sync.vhd\n" Sys.argv.(0);
    Printf.printf "  %s sysver_tests/uart_*.vhd\n" Sys.argv.(0);
    exit 1
  end;

  let files = List.tl (Array.to_list Sys.argv) in
  let ctx = convert_vhdl_files files in

  Printf.printf "\n%s\n" (String.make 70 '=');
  Printf.printf "IR Generation Summary:\n";
  Printf.printf "  Success! Generated IR for %s\n" ctx.module_name;
  if List.length ctx.nodes > 0 then begin
    ...
    List.iter (fun (id, op, inputs) ->
      ...
      Printf.printf "  $%d = %s(...)\n" id op_name
       ...
       if List.length lst > 5 then List.filteri (fun i _ -> i < 5) lst else lst)
  end

(* Run main if this is the entry point *)
let () =
  if Sys.argv.(0) |> Filename.basename |> String.equal "vhdl_to_ir_iterate.exe" then
    main ()
