...
if [ $recompile_count -lt $decompile_count ]; then
    failed=$((decompile_count - recompile_count))
    echo -e "${YELLOW}⚠ $failed files failed to re-compile${NC}"
    echo ""
    echo "Files that failed re-compilation have decompiler bugs."
    echo "Check these files for syntax errors:"
    echo ""
    
    for sv_file in "$DECOMPILED_SV_DIR"/*.sv; do
        basename=$(basename "$sv_file" .sv)
        if [ ! -f "$ROUNDTRIP_JSON_DIR/${basename}.tree.json" ]; then
            echo "  ✗ $basename"
            echo "    Debug: verilator --lint-only $sv_file"
        fi
    done
    echo ""
fi
...
if [ -f "$REPORT_FILE" ]; then
    ...
    if [ -f "$REPORT_FILE" ]; then
        perfect=$(tail -n +2 "$REPORT_FILE" 2>/dev/null | awk -F',' '$2=="true"' | wc -l)
        total=$(tail -n +2 "$REPORT_FILE" 2>/dev/null | wc -l)
        
        if [ $total -gt 0 ]; then
            echo ""
            echo "Comparison Results:"
            echo "  Perfect matches: $perfect / $total"
            
            if [ $perfect -eq $total ]; then
                echo -e "  ${GREEN}🎉 All files match perfectly!${NC}"
            else
                echo ""
                echo "  Files with differences:"
                tail -n +2 "$REPORT_FILE" 2>/dev/null | awk -F',' '$2=="false" {printf "    • %s (%.1f%% similar)\n", $1, $3*100}'
            fi
        fi
    fi
fi
...
if [ $recompile_count -eq $decompile_count ] && [ $perfect -eq $total ] 2>/dev/null; then
    echo -e "${GREEN}✓ Success! All files completed perfect round-trip.${NC}"
    exit 0
elif [ $recompile_count -gt 0 ]; then
    echo -e "${YELLOW}⚠ Partial success. $recompile_count files completed round-trip.${NC}"
    echo "Fix decompiler bugs in the failing files to improve results."
    exit 0
else
    echo -e "${RED}✗ No files completed full round-trip.${NC}"
    exit 1
fi
