...
type backend =
  ...
  | XilinxRtl     (* Structural relabeled to Vivado RTL_* primitive names *)
...
let generate_output backend ast =
  ...
  | XilinxRtl ->
      (* Same pipeline as Structural, then rename primitives to Vivado RTL_* *)
      let transformed_ast = Sv_transform.transform ~verbose:false ast in
      let structural_ast, warnings = Sv_tran_struct.generate_structural_ast transformed_ast in
      let xilinx_ast = Sv_gen_xilinx_rtl.transform structural_ast in
      let result = Sv_gen.generate_sv xilinx_ast 0 in
      let header = "// Generated by xilinx_rtl backend\n// Cells use Vivado RTL elaboration vocabulary (RTL_INV, RTL_AND, RTL_REG_SYNC, ...)\n\n" in
      (header ^ result, warnings)
  | Yosys ->
...
let get_extension backend =
  match backend with
  | Standard | Structural | XilinxRtl | Yosys | HardCaml -> ".sv"
