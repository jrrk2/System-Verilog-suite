...
let rec sigspec_to_bexpr = function
  ...
  | SigConst s ->
      (* Yosys constants come in many forms: `4'b0101`, `1'h1`, plain
       * decimal, or a raw bit string. Best-effort decode. *)
      ...
      let value, width =
        try
          if String.contains s '\'' then
            ...
            match parts with
            | [w; rest] when String.length rest >= 2 ->
...
let module_to_bmodule (m : rtlil_module) : bmodule =
  ...
  let connect_procs = List.filter_map (fun (lhs, rhs) ->
    match sigspec_to_lhs lhs with
    | Some name ->
        let r = sigspec_to_bexpr rhs in
        Some (BCombinational {
          name = "connect_" ^ name;
          sensitivity = [BAny];
          body = [BAssign { lhs = name; rhs = r }];
        })
    | None -> None
  ) m.mod_connects in
  {
