#!/bin/bash
# Run pairwise Z3 miters across three frontends (verible, slang,
# sv-parser) over per-module cva6 sources extracted from
# test/cva6_ram/cva6_flat.sv.  Skips verilator (cva6's package-
# qualified packed structs trip its parser) and synlig (segfaults on
# large preprocessed inputs).
#
# Output: $OUT/cva6_vss.csv

set -e
HERE=/home/jonathan/System-Verilog-suite
MITER=$HERE/_build/default/test_vss_miter.exe
FLAT=$HERE/test/cva6_ram/cva6_flat.sv
OUT=${OUT:-/tmp/cva6_vss}
mkdir -p "$OUT/src" "$OUT/logs"
CSV=$OUT/cva6_vss.csv

[ -x "$MITER" ] || { echo "build first: dune build test_vss_miter.exe"; exit 1; }
[ -f "$FLAT" ] || { echo "missing $FLAT"; exit 1; }

PAIRS=("verible vs slang" "verible vs sv-parser" "slang vs sv-parser")

echo "test,top,vs,vp,sp,consensus,seconds" > $CSV

# Extract leaf modules from cva6_flat.sv (skip parameter-mangled,
# vendored FPU library).
awk -v out="$OUT/src" '
...
for sv in $OUT/src/*.sv; do
    ...
    # 3 frontends, 3 pairs.  ALL-EQ = 3 pairs EQ.  Exactly one
    # dissenter = its 2 pairs NEQ.
    bad_v=0; bad_s=0; bad_p=0
    [ "${V[0]}" != "EQUIVALENT" ] && { bad_v=$((bad_v+1)); bad_s=$((bad_s+1)); }
    [ "${V[1]}" != "EQUIVALENT" ] && { bad_v=$((bad_v+1)); bad_p=$((bad_p+1)); }
    [ "${V[2]}" != "EQUIVALENT" ] && { bad_s=$((bad_s+1)); bad_p=$((bad_p+1)); }

    eq_count=0
    for v in "${V[@]}"; do [ "$v" = "EQUIVALENT" ] && eq_count=$((eq_count+1)); done

    if [ "$eq_count" -eq 3 ]; then
        consensus="EQ_ALL"; all_eq=$((all_eq+1))
    elif [ "$eq_count" -eq 1 ]; then
        if   [ "$bad_v" = 2 ]; then consensus="ODD=verible"
        elif [ "$bad_s" = 2 ]; then consensus="ODD=slang"
        elif [ "$bad_p" = 2 ]; then consensus="ODD=sv-parser"
        else consensus="ODD=?"
        fi
        one_off=$((one_off+1))
    else
...
echo
echo "==== three-way (verible/slang/sv-parser) over cva6 leaf modules ($n tests) ===="
