(* test_no_verilator_miter — 4-way pairwise miter across the four
   SystemVerilog frontends that don't need pre-rendered verilator JSON:
   verible, slang, sv-parser (dalance), synlig (Surelog).

   usage:
     test_no_verilator_miter <top> <sv_file>

   Designed for codebases verilator can't fully elaborate — e.g. cva6,
   where slang/synlig handle package-qualified packed-struct types
   that verilator's parser rejects.  Six pairwise miters; exit 0 only
   when all agree, 1 if any DIFFER, 2 if any error. *)
...
let () =
  ...
  Printf.printf "\n4-way (no-verilator) miter for %s\n" top;
  List.iter (fun (a, b, r) ->
